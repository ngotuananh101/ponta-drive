    ...
    <div class="flex flex-col gap-5">
      ...
      <div class="flex items-center justify-between flex-wrap gap-4">
        ...
        <div class="flex items-center gap-2">
          <!-- List / Grid Toggle -->
          <div class="flex items-center bg-card border border-border rounded-xl p-1 shadow-xs">
            <Button
              variant="ghost"
              size="icon"
              class="h-8 w-8 rounded-lg"
              :class="{ 'bg-muted text-primary shadow-xs font-bold': viewMode === 'list' }"
            ...
            <Button
              ...
              size="icon"
              class="h-8 w-8 rounded-lg"
              :class="{ 'bg-muted text-primary shadow-xs font-bold': viewMode === 'grid' }"
          ...
          <Button
            variant="ghost"
            size="icon"
            class="h-9 w-9 rounded-xl border border-border bg-card shadow-xs"
      ...
      <div class="flex items-center gap-2 overflow-x-auto pb-1 scrollbar-none">
        <button
          type="button"
          class="flex items-center gap-1.5 px-3.5 py-1.5 rounded-full border border-border bg-card hover:bg-accent/60 text-xs font-medium text-foreground transition-colors shrink-0 shadow-xs cursor-pointer"
        >
          <span>{{ t('drive.filter_type') }}</span>
        ...
        <button
          ...
          class="flex items-center gap-1.5 px-3.5 py-1.5 rounded-full border border-border bg-card hover:bg-accent/60 text-xs font-medium text-foreground transition-colors shrink-0 shadow-xs cursor-pointer"
        >
          <span>{{ t('drive.filter_people') }}</span>
          <ChevronDown class="h-3.5 w-3.5 text-muted-foreground" />
        </button>
        ...
        <button
          ...
          class="flex items-center gap-1.5 px-3.5 py-1.5 rounded-full border border-border bg-card hover:bg-accent/60 text-xs font-medium text-foreground transition-colors shrink-0 shadow-xs cursor-pointer"
        >
          <span>{{ t('drive.filter_modified') }}</span>
          <ChevronDown class="h-3.5 w-3.5 text-muted-foreground" />
        </button>
        ...
        <button
          ...
          class="flex items-center gap-1.5 px-3.5 py-1.5 rounded-full border border-border bg-card hover:bg-accent/60 text-xs font-medium text-foreground transition-colors shrink-0 shadow-xs cursor-pointer"
        >
          <span>{{ t('drive.filter_source') }}</span>
          <ChevronDown class="h-3.5 w-3.5 text-muted-foreground" />
        </button>
      </div>
      ...
      <div class="flex gap-4">
        ...
        <div class="flex-1 min-w-0">
          ...
          >
            <!-- Table Header -->
            <div
              class="grid grid-cols-12 px-4 py-3 border-b border-border text-xs font-semibold text-muted-foreground select-none"
            >
            ...
            <div class="divide-y divide-border/60">
              <div
                v-for="item in items"
                :key="item.id"
                class="grid grid-cols-12 px-4 py-3 items-center text-sm transition-colors cursor-pointer group"
                :class="[
              ...
              >
                <!-- Column: Name & Icon -->
                <div class="col-span-6 sm:col-span-6 flex items-center gap-3 min-w-0 pr-2">
                  <!-- Folder Icon -->
                  <div v-if="item.type === 'folder'" class="shrink-0">
                    <Folder class="h-5 w-5" :class="item.folderColor || 'text-zinc-400'" />
                  </div>
                  <!-- Document Icon -->
                  ...
                  <DropdownMenu>
                    <DropdownMenuTrigger as-child>
                      <Button
                        variant="ghost"
                        size="icon"
                        class="h-7 w-7 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity"
                        @click.stop
                      >
        ...
        <!-- Details Sidebar Panel (Toggleable) -->
        <aside
          v-if="showDetails"
          class="w-72 rounded-2xl border border-border bg-card p-4 shrink-0 hidden md:flex flex-col gap-4 shadow-xs"
        >
